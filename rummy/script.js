...
// Game Variables
let player = { x: canvas.width / 2 - 20, y: canvas.height - 120, width: 40, height: 80, color: "#00ffcc", speed: 5 };
let enemyCars = [];
let roadMarkers = [];
let powerUps = [];
let score = 0;
...
function createEnemyCar() {
    const laneX = [100, canvas.width / 2 - 20, canvas.width - 140]; // Lane positions adjusted for responsive canvas
...
window.addEventListener("keydown", e => {
    ...
    if (e.key === "ArrowRight" && player.x < canvas.width - 120) player.x += 80;
});
...
window.addEventListener('touchmove', e => {
    ...
    if (touchEndX < touchStartX && player.x > 100) {
        player.x -= 80; // Move left
    } else if (touchEndX > touchStartX && player.x < canvas.width - 120) {
        player.x += 80; // Move right
    }
});
...
setInterval(() => {
    const laneX = [100, canvas.width / 2 - 20, canvas.width - 140];
    const randomLane = laneX[getRandomInt(0, laneX.length - 1)];
